...
import { addReviews, clearErrors } from "redux/actions/reviewAction";
...
const SumitReviews = () => {
    ...
    const onSubmit = (data, e) => {
        if (rating === 0) {
            ...
            dispatch(addReviews(Router.query.index, data.message, rating))
        }
    };
    const { loading, success, message } = useSelector(state => state.addReducer);
    ...
    return (
        <>
            <Box as="form" onSubmit={handleSubmit(onSubmit)}>
                ...
                {btnType === "submit" &&
                    <Button variant="SubmitButtons" type='submit' sx={{ width: ["100%", null, "40%", "10%"] }}>
                        {loading ?
                            <Image sx={styles.Loading} src={Loader} alt="Loading" /> : "Submit"
                        }
                    </Button>
                }
            </Box>
