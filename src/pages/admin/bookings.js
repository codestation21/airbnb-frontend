...
import { checkAdmin, getAllAdminBookings } from "redux/actions/adminAction";
import SingleNav from "sections/Single/SingleNav";
import Heads from "utilis/Heads";
import AllBookings from "sections/Admin/allBookings";

const Booking = () => {
    const { message } = useSelector(state => state.getAdminBooking);
    return (
...
export const getServerSideProps = wrapper.getServerSideProps(
    (store) =>
        async (context) => {
            ...
            await store.dispatch(getAllAdminBookings(context.req.cookies['token']));
        }
)